...
__attribute__((section("getsockopt"), used)) int
lightpath_getsocketopt(struct bpf_sockopt *ctx) {
  ...
  if (conn.family == AF_INET) { // IPv4
    ...
    *(struct sockaddr_in *)ctx->optval = socket_addr4;
    ...
    if ((void *)((struct sockaddr_in6 *)ctx->optval + 1) <= ctx->optval_end) {
      errork(
          "Optname SO_ORIGINAL_DST invalid getsockopt optval size (AF_INET6)");
      return 1;
    }
