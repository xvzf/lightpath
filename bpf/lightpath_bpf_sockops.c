...
__attribute__((section("sockops"), used)) int lightpath_sock_connect(struct bpf_sock_ops *ctx)
{
  __u64 cookie;
  ...
  if (!orig_dst)
  {
    // No cookie exists -> either we're not interested in this connection or the socket connection failed
    return 0;
  }
  ...
  if (!bpf_map_update_elem(&conn_pair_to_original_dst_map, &conn, orig_dst, BPF_ANY))
  {
    errork("Failed updating original dst for connection pair");
  }

  // Update socket pair map
  if (!(bpf_sock_hash_update(ctx, &conn_pair_to_sock_map, &conn, BPF_NOEXIST)))
  {
