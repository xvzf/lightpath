...
__attribute__((section("connect"), used)) int
lightpath_sock_connect(struct bpf_sock_addr *ctx) {
  ...
  // Check if IP is a cluster IP
  {
    ...
    if (res) {
      ...
      debugk("Not a cluster IP: %d.%d.%d.%d", dst_ip[0], dst_ip[1], dst_ip[2],
             dst_ip[3]);
      return 0;
    }
  }
  ...
  debugk("cgroup/connect{4,6}: cookie: %d, ip: %x:%x:%x:%x, port: %d", cookie,
         dst_ip[0], dst_ip[1], dst_ip[2], dst_ip[4], ctx->user_port);
  ...
  // Extract destination information
  {
    ...
    // Save in LRU hashmap
    if (bpf_map_update_elem(&cookie_to_original_dst_map, &cookie, &original_dst,
                            BPF_ANY)) {
